...
`);return`mailto:${e}?subject=Shopify%202.0%20rebuild%20lead&body=${encodeURIComponent(n)}`}function o(e,t){i&&(i.textContent=e,i.classList.remove(`ok`,`err`),t&&i.classList.add(t))}r?.addEventListener(`submit`,async e=>{e.preventDefault();let n=new FormData(r);if(String(n.get(`_gotcha`)||``)||String(n.get(`_honey`)||``)){o(`Sent.`,`ok`);return}let i={name:String(n.get(`name`)||``).trim(),email:String(n.get(`email`)||``).trim(),store_url:String(n.get(`store_url`)||``).trim(),offer:String(n.get(`offer`)||``).trim(),whats_broken:String(n.get(`whats_broken`)||``).trim(),_subject:`Shopify 2.0 rebuild lead`};s=r.querySelector(`button[type="submit"]`);s&&(s.disabled=!0),o(`Sending…`,``);try{let e=await fetch(t,{method:`POST`,headers:{"Content-Type":`application/json`,Accept:`application/json`},body:JSON.stringify(i)});if(!e.ok)throw Error(`HTTP ${e.status}`);r.classList.add(`sent`),o(`Got it. If this is the first form from this page, I still have to confirm FormSubmit in Proton — use the mailto if you need it today.`,`ok`)}catch{o(`FormSubmit didn’t take it. Opening a mailto draft with the same details.`,`err`),window.location.href=a(i),s&&(s.disabled=!1)}});var s=document.querySelector(`#split-frame`),c=document.querySelector(`#split-range`);function l(e){s&&(s.style.setProperty(`--pos`,`${e}%`),c?.setAttribute(`aria-valuenow`,e))}c?.addEventListener(`input`,()=>l(c.value)),l(c?.value??`58`);var u=document.querySelector(`#ht-stage`),d=document.querySelector(`#ht-note`),htSticky=document.querySelector(`#ht-sticky-atc`),htStickyLabel=document.querySelector(`#ht-sticky-label`),htCartCount=document.querySelector(`#ht-cart-count`),htCartN=0;function f(e){if(!u)return;let t=()=>{u.querySelectorAll(`[data-view]`).forEach(t=>{t.classList.toggle(`is-on`,t.dataset.view===e)}),u.querySelectorAll(`[data-ht-nav]`).forEach(t=>{t.classList.toggle(`is-on`,e===`home`?t.dataset.htNav===`home`:t.dataset.htNav===e)});if(htSticky){var onPdp=e!==`home`;htSticky.hidden=!onPdp;if(onPdp&&htStickyLabel){var title=u.querySelector(`[data-view="${e}"] .ht-buy h3`);htStickyLabel.textContent=title?title.textContent:`Sample SKU`}}document.documentElement.classList.add(`ht-active`)},r=document;!n&&typeof r.startViewTransition==`function`?r.startViewTransition(t):t()}document.querySelectorAll(`[data-ht]`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.dataset.ht;t&&f(t)})}),document.querySelectorAll(`[data-ht-cart]`).forEach(e=>{e.addEventListener(`click`,()=>{htCartN+=1;if(htCartCount)htCartCount.textContent=String(htCartN);d&&(d.hidden=!1,d.textContent=`Sample store. Not for sale — this is the 2.0 rebuild, not a checkout.`)})});document.querySelectorAll(`[data-gallery]`).forEach(function(g){var primary=g.querySelector(`[data-primary]`);g.querySelectorAll(`.ht-thumb`).forEach(function(thumb){thumb.addEventListener(`click`,function(){if(!primary)return;primary.src=thumb.dataset.src;if(thumb.dataset.alt)primary.alt=thumb.dataset.alt;g.querySelectorAll(`.ht-thumb`).forEach(function(t){t.classList.toggle(`is-on`,t===thumb)})})})});document.querySelectorAll(`[data-scroll-shop]`).forEach(function(btn){btn.addEventListener(`click`,function(){var shop=document.querySelector(`#ht-shop`);if(shop)shop.scrollIntoView({behavior:n?`auto`:`smooth`,block:`start`})})});
(function(){
  ...
  var hearthSec = document.getElementById('hearth');
  if(hearthSec && 'IntersectionObserver' in window){
    var htDockObs = new IntersectionObserver(function(entries){
      entries.forEach(function(entry){
        document.documentElement.classList.toggle('ht-active', entry.isIntersecting);
      });
    }, { rootMargin: '-10% 0px -18% 0px', threshold: 0.08 });
    htDockObs.observe(hearthSec);
  }
  if(location.hash === '#hearth'){
    document.documentElement.classList.add('ht-active');
  }
